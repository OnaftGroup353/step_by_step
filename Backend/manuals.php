...
function getManualById()
{
    ...
    if (!isset($api->_request->id))
        $api->send_error(101);
    $id = $api->_request->id;
	$query="SELECT * FROM `manual_articles` WHERE `id`=".$id;
	$r = $api->db_conn->query($query) or die($api->db_conn->error);
	$row = $r->fetch_assoc();
	...
	$res = (object) array("date" => $date, "chapters" => $chapters, "literatures" => $literatures, "tableOfContents" => $tableOfContents, "metadata" => $metadata);
	...
	if($r->num_rows > 0) 
	{
		while($res = $r->fetch_assoc()) 
		{
			if ($res["article_type_id"] == 2)
			{
				...
				$child = (object) array ("name" => $res["caption"]);
				$i = 1;
			...
			{
				$child = (object) array("type" => $res["article_type"], "title" => $res["caption"], "data" => $res["content"]);
				$ress[] = $child;
			}
		}
	}
