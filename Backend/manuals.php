...
function articleSearch()
{
    ...
    $query = "select ma2.id, a3.caption, UNIX_TIMESTAMP(a3.`update_date`) as `date`
from manual_articles ma2
  INNER join articles a3
  on a3.id = ma2.article_id
where ma2.article_id in (
SELECT DISTINCT a.id
FROM articles a
  INNER JOIN manual_articles ma
    ON ma.article_id = a.id
WHERE a.article_type_id = 1 AND (a.caption LIKE CONCAT('%', '$name', '%') OR a.content LIKE CONCAT('%', '$name', '%'))
UNION
(SELECT DISTINCT a2.id
 FROM articles a
   ...
   INNER JOIN articles a2
     ON a2.id = ma.parent_article_id
        AND a2.article_type_id = 1
 WHERE a.article_type_id = 2 AND (a.caption LIKE CONCAT('%', '$name', '%') OR a.content LIKE CONCAT('%', '$name', '%')))
UNION
(SELECT DISTINCT mt.manual_id
 FROM articles a
   INNER JOIN manual_tags mt
     ON mt.manual_id = a.id
 WHERE a.article_type_id = 1 AND mt.tag like CONCAT('%', '$name', '%')))";
$r = $api->db_conn->query($query) or die($api->db_conn->error . " " . __LINE__);
    if($r->num_rows > 0)
    {
        $rows = array();
