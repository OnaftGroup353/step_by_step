...
function update_user_email()
{
	...
	*/
	...
	$time = microtime(true);
	$confirmationCode = bin2hex(openssl_random_pseudo_bytes($length)).md5($time);
	$query="UPDATE users SET `email`='$email', `mailconfimationcode`='$mailconfimationcode',`ismailconfirmed`='0'  WHERE id=$id";
	$r = $api->db_conn->query($query) or die($api->db_conn->error);
	if ($r)
	{
		send_mail($email, $confirmationCode);
...
function confirmEmail()
{
	...
	*/
	...
	if (strrpos($confirmationCode, "'"))
		$api->send_error("Invalid confirmation code!", 400);

	$query="SELECT count(*) as COU FROM users WHERE `confirmationCode`='$confirmationCode'";
	$r = $api->db_conn->query($query) or die($api->db_conn->error);
	if (!$r)
		$api->send_error('Internal Server Error',500);
	$res = $r->fetch_assoc();
	...
	$query="UPDATE users SET `ismailconfirmed`='1'  WHERE `confirmationCode`='$confirmationCode'";
	$r = $api->db_conn->query($query) or die($api->db_conn->error);
	if ($r)
		$api->response("OK", 200, "text");
	$api->send_error('Internal Server Error',500);
