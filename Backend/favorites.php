...
function addFavorite()
{
	...
	FROM `favorite_manuals`
	WHERE `user_id`='$user_id' AND article_id = '$manual_id'";
...
function getMyFavorites()
{
    ...
    $session = $api->getSessionData($api->_request->token);
	$user_id = $session["user_id"];
	$query="SELECT fav.`article_id`, a.`caption`, UNIX_TIMESTAMP(a.`update_date`) as `date`
	FROM (`favorite_manuals` as fav INNER JOIN `manual_articles` as ma ON fav.`article_id`=ma.`id`)
									INNER JOIN `articles` as a ON ma.`article_id`=a.`id`
	...
	if ($r)
	{
		if ($r->num_rows > 0)
		{
			...
			{
				$rrr = (object) array("id" => $res["article_id"], "caption" => $res["caption"], "date" => $res["date"]);
				$response[] = $rrr;
			}
